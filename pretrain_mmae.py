...
from models.croco import MMAE_CroCoNet
...
def get_args_parser():
    ...
    # model and criterion
    parser.add_argument('--model', default='MMAE_CroCoNet()', type=str, help="string containing the model to build")
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (image1, image2) in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        with torch.cuda.amp.autocast(enabled=bool(args.amp)):
            out1, mask1, target1, out2, mask2, target2 = model(image1, image2)
