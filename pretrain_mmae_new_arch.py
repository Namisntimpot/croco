...
def get_args_parser():
    ...
    # model and criterion
    parser.add_argument('-cfg', '--config', type=str, help="path to the config file")
...
def main(args):
    ...
    if args.output_dir:
        megfile.smart_makedirs(args.output_dir, exist_ok=True)
        # Path(args.output_dir).mkdir(parents=True, exist_ok=True)
    megfile.smart_copy(args.config_path, os.path.join(args.output_dir, 'config.yaml'))
    ...
    ## model 
    print('Loading model: {:s}'.format(args.config.name))
    model_config = args.config.model
    model = make_model(model_config.task, model_config.pretrain_type, model_config.img_size, **model_config.kwargs)
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (image1, image2) in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        with torch.autocast('cuda', enabled=bool(args.amp)):
            out1, mask1, target1, out2, mask2, target2 = model(image1, image2, do_mask=True)
            loss = (criterion(out1, mask1, target1) + criterion(out2, mask2, target2)) / 2
