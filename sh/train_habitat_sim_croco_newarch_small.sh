...
# output_dir="exp/croco_newarch-small-habitat_sim-mask_0.8"
output_dir=$2

mkdir -p $output_dir

echo "torchrun --nproc_per_node=$num_gpu pretrain_croco_new_arch.py -cfg $config_path --dataset $datasets --output_dir $output_dir --data_dir $data_dir \
...
if [[ "$platform" == "cluster" ]]; then
    ...
    tlaunch --gpu=$num_gpu --cpu=$num_cpu --memory=$memory --positive-tag=$gpu_type -- \
        torchrun --nproc_per_node=$num_gpu pretrain_croco_new_arch.py -cfg $config_path --dataset $datasets --output_dir $output_dir --data_dir $data_dir \
